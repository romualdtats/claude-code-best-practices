...
#### 5. 来源信息提取
- **作者信息**: 从源内容或用户提供的信息中提取原作者
- **来源URL**: 获取原始文章的完整链接
- **发布日期**: 提取或确定文章的发布日期
- **版权信息**: 记录任何相关的版权声明或使用许可

#### 6. 英文文档创建
- **目标路径**: `content/docs/en/best-practices/`
- **文件格式**: `.mdx` (支持React组件)
- **Front Matter**: 包含标题、描述、语言标识、作者、来源、日期
- **内容结构**: 
  ```yaml
  ---
  ...
  ---
  ```
- **来源信息**: 自动从源内容或用户提供的信息中提取作者、来源URL和发布日期
...
- **翻译原则**: 
  ...
  - 保留代码示例不变
  - 维持原文档结构
- **Front Matter**: 包含相同的作者、来源、日期信息
- **来源信息**: 保持与英文版本一致的作者、来源URL和发布日期
- **质量控制**: 技术概念对照检查
...
### 代码和提示词保护
- [ ] 代码示例和代码块保持原样，未做任何修改
- [ ] 提示词（Prompt）内容完全保持原始状态
- [ ] 子代理（Sub Agent）定义未做任何更改
- [ ] 技术配置和参数设置保持原始值
- [ ] 仅对文章结构和说明文字进行优化，不修改技术内容
